...
#
# CACHED (~30 min): the getToken endpoint is rate-limited (`-32400 Превышено число попыток
# операции`), so minting on every `cd` trips the limit and stalls the shell. We mint at most
# ~twice an hour and read the cache the rest of the time. The cache read is OUTSIDE the fetch
# guard so an existing token is exported even when the CLI can't be invoked here.
#
...
if [ ! -s "$__sweb_cache" ] || [ -n "$(find "$__sweb_cache" -mmin +30 2>/dev/null)" ]; then
